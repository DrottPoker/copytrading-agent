    ...
    <section className="ui-panel overflow-hidden">
      <div className="flex flex-col gap-3 border-b border-line px-4 py-3 lg:flex-row lg:items-center lg:justify-between">
        <div className="flex min-w-0 items-start gap-2">
          <Scissors className="mt-0.5 h-4 w-4 shrink-0 text-muted" aria-hidden="true" />
          <div>
    ...
    <div className="rounded-md border border-line bg-subtle p-3">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <p className="mt-1 text-sm text-muted">{rule.rule}</p>
        </div>
      ...
      <div className="mt-3 max-h-96 overflow-y-auto divide-y divide-line rounded-md border border-line bg-white px-3">
        {candidates.length === 0 ? (
          <p className="py-4 text-sm text-muted">No wallets matched.</p>
        ) : (
    ...
    <div className="py-3">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <p className="font-mono text-xs text-muted">{shortAddress(item.address)}</p>
        </div>
        <div className="shrink-0 text-right">
          <p className="font-mono text-xs font-semibold text-ink">{candidateScore(item)}</p>
          <p className="mt-1 font-mono text-xs text-muted">{candidateValue(item)}</p>
        </div>
      </div>
      <p className="mt-2 text-xs text-muted">{candidateDetail(item)}</p>
    </div>
  );
}
